...
class GerenciadorDeTarefas:
    ...
    def atualizar_tarefas(self, id, nome, descricao, data_limite, prioridade):
        query = '''
                    ...
                    WHERE id = ?
                '''
        self.cursor.execute(query,(nome, descricao, data_limite, prioridade, id))
        self.conn.commit()
...
menu = st.sidebar.selectbox("Menu", ["Adicionar Tarefa","Visualizar Tarefas","Atualizar Tarefa","Excluir Tarefa"])
...
if menu == "Adicionar Tarefa":
    ...
    tarefas = db.listar_tarefas()
    if tarefas:
        tarefa_id = st.selectbox("Selecione a tarefa a ser atualizada.",[tarefa.id for tarefa in tarefas])
        tarefa = next(tarefa for tarefa in tarefas if tarefa.id == tarefa_id)

        with st.form("form_atualizar"):
            nome = st.text_input("Nome da tarefa", value=tarefa.nome)
            descricao = st.text_input("Descrição da tarefa", value=tarefa.descricao)
            data_limite = st.date_input("Prazo: ", value=datetime.strptime(tarefa.data_limite, "%Y-%m-%d"))
            prioridade = st.selectbox("Prioridade", ["Alta","Média","Baixa"], index=["Alta","Média","Baixa"].index(tarefa.prioridade))
            submit = st.form_submit_button("Atualizar")

            if submit:
                db.atualizar_tarefas(tarefa_id, nome, descricao, str(data_limite), prioridade)
    ...
    tarefas = db.listar_tarefas()
    if tarefas:
        tarefa_id = st.selectbox("Selecione a tarefa para ser excluída", [tarefa.id for tarefa in tarefas])
